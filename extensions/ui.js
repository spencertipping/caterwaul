...
//   quotes a compiled pattern.
             ...
             -where [jq            = options && options.jquery_name || 'jQuery',
                     ...
                     jquery_macros = [rule('J[_element]',                 '#{jq}(TS[_element])'),
                                      ...
                                      rule('J[_element >> _pattern]',     '(J[_element]).filter((PS[P[_pattern]]))'),
                                      rule('J[_element >>> _pattern]',    '(J[_element]).find((PS[P[_pattern]]))'),
                                      rule('J[_element << _pattern]',     '(J[_element]).parents((PS[P[_pattern]]))'),
                     ...
                     search_macros = [rule('P[_element]',                 new $.syntax(hyphenate(match._element.data -re [it === '_' ? '*' : it]))                      -given.match),
                                      ...
                                      rule('P[_attribute = _value]',      new $.syntax('#{this.expand(p(match._attribute)).data}="#' + '{#{match._value.toString()}}"') -given.match),
                              ...
                              -where [binary(op)(match) = new $.syntax('#{this.expand(p(match._element1)).data}, #{this.expand(p(match._element2)).data}')]]})(caterwaul);
// Generated by SDoc 
