...
//   | xs.fold($+$)
    ...
    tconfiguration('qs qg fn', 'op.fn', function () {for (var ops = qw('+ - * / & | ^ && || % < > << >> >>> , in instanceof'), i = 0, l = ops.length, op; op = ops[i], i < l; ++i)
                                                       this.macro(new this.syntax(op, '$', '$'), fn_[qs[qg[function (x, y) {return _}]].s('_', new this.syntax(op, 'x', 'y'))])}).
