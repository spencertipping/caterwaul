...
//     descend into the nodes beyond the one specified by the flatten() call.
                 ...
                 flatten: function () {var d = this.data;
                                       return ! (has(parse_lr, d) && this.length) ? this : has(parse_associates_right, d) ?
  ...
  associator_for = function (f) {return function (name, behavior, value) {return f[name] = (f.behaviors[f.attributes[name] = behavior] || id).call(f, value), f}},
    shallow_copy = function (x) {return x && (x.constructor === Array ? Array.prototype.slice.call(x) : merge({}, x))},
         copy_of = function (f) {var g = merge(function () {return g.init.apply(g, arguments)}, {behaviors: shallow_copy(f.behaviors), attributes: {}});
              ...
              $c = this.caterwaul = merge(copy_of({behaviors: {method: function (v) {return bind(v, this)}}}),
                                          {deglobalize: function () {_global.caterwaul = _caterwaul; return this}}).

//   Bootstrapping method behavior.
//   Setting up the behavior(), method(), ref(), and shallow() methods.

    behavior('ref').behavior('shallow', shallow_copy).
...
// Now that we've defined lexing, parsing, and macroexpansion, we can create a global Caterwaul function that has the appropriate attributes.
    ...
    shallow('compiler', {qs: parse(lex('qs[_]'))}).shallow('macro_patterns', []).shallow('macro_expanders', []).
     ...
     method('expand',    fn('@expand_qs(@macroexpand($0))')).
     method('macro',     fn('@macro_patterns.push($0), @macro_expanders.push($1), this')).
...
//   There are several shorthands that are useful for functions. fn[x, y, z][e] is the same as function (x, y, z) {return e}, fn[][e] constructs a nullary function returning e. Also includes
...
//   means that you'll get the whole comma-expression placed inside a function, rendering it useless for expressions inside procedure calls. (You'll need parens for that.)
    ...
    ref('fn', function () {this.rmacro(qs[fn[_][_]],    function (vars, expression) {return qs[(function (_) {return _})].s('_', [vars, expression])});
                           this.rmacro(qs[let[_] in _], function (vars, expression) {if (vars.data === ',') vars = vars.flatten();
                           ...
                           this.rmacro(qs[_, where[_]], function (expression, vars) {return qs[(let[_] in (_))].s('_', [vars, expression])})})}) ();
