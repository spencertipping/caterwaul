...
//       besides, using macros for this is inefficient anyway.)
      ...
      {annotate: function (node) {return node && (node.is_number() || node.is_string() || node.is_boolean() || node.is_regexp() || node.data === 'null' || node.data === 'undefined' ? node :
                    node.length === 0  ?  recon.wrap(node, node) :
                 /=$/.test(node.data)  ?  recon.wrap(node, new s(node.data, node[0].data === '[]' ? new s('[]', recon.annotate(node[0][0]), new s('[', recon.annotate(node[0][1][0]))) :
                                                                            node[0].data ===  '.' ? new s('.',  recon.annotate(node[0][0]), node[0][1]) : node[0], recon.annotate(node[1]))) :

                   node.data === '()'  ?  recon.wrap(node, node[0].data !== '[]' && node[0].data !== '.' ?
                                            new s('()', recon.annotate(node[0]), new s('(', recon.annotate(node[1][0]))) :
                  ...
                  node.data === 'var'  ?  new s('var', node[0].data === ',' ? new s(',', node[0].flatten().map(function (n) {return n.data === '=' && new s(n.data, n[0], recon.annotate(n[1]))})) :
                                                       node[0].data === '=' ? new s('=', node[0][0], node[0][1]) : node[0]) :
             node.data === 'function' ||
                node.data === 'catch'  ?  new s(node.data, node[0], recon.annotate(node[1])) :
