               ...
               syntax_node = extend(function (data) {this.data = data; this.length = 0; this.l = this.r = this.p = null;
                ...
                reparent: fn('@p && @p[0] === this && (@p[0] = $0), this'),                         fold_l: fn('@l && @append(@l.unlink(this)), this'),  fold_lr: fn('@fold_l().fold_r()'),
                 ...
                 sibling: fn('$0.p = @p, (@r = $0).l = this'),                                      unlink: fn('@l && (@l.r = @r), @r && (@r.l = @l), @l = @r = null, @reparent($0)'),
                    wrap: fn('$0.p = @replace($0).p, @reparent($0), @l = @r = null, @append_to($0)'),  pop: fn('--@length, this'),  push: fn('$0 && (this[@length++] = $0), this'),
...
//   tree of the form (+ 'string' (expression) 'string' (expression) ... 'string') -- that is, a flattened variadic +. Strings that do not contain #{} groups are returned as-is.
    ...
    configuration('string', function () {this.configure('fn').rmacro(qs[_], function (s) {
                                          ...
                                          for (var i = 0, l = strings.length - 1; i < l; ++i) result.push(new this.syntax(q + strings[i] + q)).push(new this.syntax('(', this.parse(xs[i])));
                                          return new this.syntax('(', result.push(new this.syntax(q + strings[strings.length - 1] + q)))})})}) ();
