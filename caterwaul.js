...
parse_associates_right = hash('= += -= *= /= %= &= ^= |= <<= >>= >>>= ~ ! new typeof u+ u- -- ++ u-- u++ ? if else function try catch finally for switch case with while do'),
   ...
   parse_r_until_block = annotate_keys({'function':2, 'if':1, 'do':1, 'catch':1, 'try':1, 'for':1, 'while':1}),
         parse_accepts = annotate_keys({'if':'else', 'do':'while', 'catch':'finally', 'try':'catch'}), parse_show = hash(';'),
...
//     it reflects the precedence of dereferencing and invocation).

     else if (has(parse_ambiguous_group, data) && node.l && (node.l.data === '.' ||
                   ! (has(lex_op, node.l.data) || has(parse_not_a_value, node.l.data))))  node.l.wrap(new_node(new syntax_node(data + parse_group[data]))).p.fold_r();
...
//   Rebase provides an infix function operator >$> that can be more readable, if more ambiguous, then Caterwaul's fn[][]. Enabling this configuration enables this notation from within Caterwaul.
    ...
    configuration('dfn', function () {this.rmacro(qs[_ >$> _], function (vars, expansion) {return qs[function (_) {return _}].s('_', [vars.data === '(' ? vars[0] : vars, expansion])})}).
...
//     Doing this can be useful for debugging macros, configurations, or other things.
    ...
    configuration('recon', function () {var old_init = this.init, s = this.syntax, hook_name = this.gensym(), gensym = this.gensym, recon = merge(function (tree) {return recon.annotate(tree)},
...
//       besides, using macros for this is inefficient anyway.)
      ...
      {annotate: function (node) {return node && (node.length === 0 ? recon.wrap(node, node) :
                                               /=$/.test(node.data) ? recon.wrap(node, new s(node.data, node[0].data === '[]' ? new s('[]', recon.annotate(node[0][0]),
                                                                                                                                            new s('[', recon.annotate(node[0][1][0]))) :
                                                                                                        node[0].data ===  '.' ? new s('.',  recon.annotate(node[0][0]), node[0][1]) : node[0],
                                                                                                        recon.annotate(node[1]))) :

node.data === '()' && node[0].data !== '[]' && node[0].data !== '.' ? recon.wrap(node, new s('()', recon.annotate(node[0]), new s('(', recon.annotate(node[1][0])))) :

                                                 node.data === '()' ? recon.wrap((function (gensym) {return qs[qg[function () {var _ = _; return _.call(_, _)}]()].
                                                                                   s('_', [gensym, recon.annotate(node[0][0]), new s(node[0].data, gensym,
                                                                                           node[0].data === '[]' ? new s('[]', gensym, new s('[', recon.annotate(node[0][1][0]))) :
                                                                                                                   new s('.',  gensym, node[0][1])), gensym,
