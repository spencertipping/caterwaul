...
//   child's parent to be updated frequently if it were there. By omitting it the logic for fold-right becomes much simpler, which should translate into (immeasurably) shorter runtimes.
                ...
                parse_link = extend (fn('this.l = this.r = null, this.data = $0, this.first = this.last = null'), {
                              fold_l: fn('this.append(this.l.remove())'),   after: fn('($1 = $0.r = this.r) && ($1.l = $0), ($0.l = this).r = $0, $0.parent = this.parent, this'),
                              fold_r: fn('this.append(this.r.remove())'),  before: fn('($1 = $0.l = this.l) && ($1.r = $0), ($0.r = this).l = $0, $0.parent = this.parent, this'),
                           fold_both: fn('this.fold_l().fold_r()'),        append: fn('this.last ? this.last.after(this.last = $0) : (this.first = this.last = $0).parent = this, this'),
                           append_to: fn('$0.append(this), this'),         remove: fn('($0 = this.l) && (this.l.r = this.r), this.r && (this.r.l = $0), this.l = this.r = this.parent = null, this'),
                                                                    insert_before: fn('$0.before(this), this'),
                                                                     insert_after: fn('$0.after(this), this')}),
...
//   transformation and quasiquoting far simpler later on.
               ...
               syntax_node = $c.syntax = extend (fn('this.data = $0, this.xs = $1'), {}),
...
//   what the parse_inverse_order table does: it maps operator names to parse_reduce_order subscripts. (e.g. 'new' -> 1.)
      ...
      for (var i = 0, l = ts.length, _; _ = ts[i], i < l; ++i)  has(parse_group, _) ? (grouping_stack.push(gs_top = parse_group[_]), head = new parse_link(_).append_to(head)) :
                                                                _ == gs_top         ? (grouping_stack.pop(),                         head = head.parent()) :
                                                                                      (head = new parse_link(_).insert_after(head),
                                                                                       has(parse_inverse_order, _) && indexes[parse_inverse_order[_]].push(head));

//   Second step: fold operators.
//   Now we can go through the list of operators, folding each according to precedence and associativity.
