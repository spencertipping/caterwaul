...
//   | let*[f = fn[x][x > 0 ? f(x - 1) + 1 : x]] in f(5)
    ...
    tconfiguration('std.qs std.qg std.fn', 'std.bind', function () {
      this.rmacro(qs[let[_] in _], fn[vars, expression][vars.data === ',' && (vars = vars.flatten()),
           ...
           rmacro(qs[let*[_] in _], fn[vars, expression][qs[qg[function () {var _; return _}].call(this)].s('_', [vars, expression])]).
           rmacro(qs[_, where*[_]], fn[expression, vars][qs[qg[let*[_] in qg[_]]].s('_', [vars, expression])])}).
...
//   This loads all of the production-use extensions.
    ...
    configuration('std', function () {this.configure(this.util.qw('std.qs std.qg std.bind std.cond std.fn std.dfn std.op.fn std.defmacro std.with_gensyms std.ref std.compile_eval std.string'))})});
