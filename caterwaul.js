        ...
        parse_reduce_order = map(hash, ['[] . ()', 'new', 'u++ u-- ++ -- typeof ~ ! u+ u-', '* / %', '+ -', '<< >> >>>', '< > <= >= instanceof in', '== != === !==', '&', '^', '|', '&&', '||',
                ...
                parse_link = extend (fn('this.left = this.right = null, this.data = $0, this.children = $1 || []'), {
                             ...
                             fold_right: fn('this.append(this.right.remove())'), before: fn('$0.left = this.left, ($0.right = this).left = $0, this'),
                              fold_both: fn('this.fold_left().fold_right()'),    append: fn('this.children = this.children.concat(Array.prototype.slice.call(arguments)), this'),
                                                                                 remove: fn('($0 = this.left) && (this.left.right = this.right), this.right && (this.right.left = $0), this')}),
               parse_group = {'(':')', '[':']', '{':'}', '?':':'},

    parse = function (ts) {
      var grouping_stack = [], root = new parse_link(), indexes = {}; 
    };
