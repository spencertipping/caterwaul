...
//   });
  ...
  gensym = (function (n) {return function () {return 'gensym' + (++n).toString(36)}})(0),  qw = fn('$0.split(/\\s+/)'),
   ...
   merge = function (o) {for (var i = 1, l = arguments.length, _ = null; _ = arguments[i], i < l; ++i) for (var k in _) has(_, k) && (o[k] = _[k]); return o},
  extend = function (f) {merge.apply(null, [f.prototype].concat(Array.prototype.slice.call(arguments, 1))); return f},
      se = function (x, f) {return f && f(x) || x},
...
//   | .associate('attribute', 'behavior', value)          Creates an attribute with the given behavior and assigns it a value. Calls the association handler on the new value.
//     ._associate('attribute', 'behavior', value)         Creates an attribute with the given behavior and assigns it a value, but doesn't call the association handler.
...
//   performance to be constant-time in the number of copies.
  ...
  associator_for = function (f) {return function (name, behavior, value) {return f._associate(name, behavior, f.behaviors[behavior].call(f, value))}},
    assigner_for = function (f) {return function (name, behavior, value) {return f.attributes[name] = behavior, f[name] = value, f}},
    shallow_copy = function (x) {return x ? x.constructor === Array ? Array.prototype.slice.call(x) : merge({}, x) : x},
         copy_of = function (f) {var g = merge(function () {return g.init.apply(g, arguments)}, {behaviors: shallow_copy(f.behaviors), attributes: {}});
                                 return se(g, function (g) {(g._associate = assigner_for(g), g.associate = associator_for(g))('behavior', 'method', function (name, definition) {
                                                              return this.associate(name, 'method', function (attribute, value) {
                                                                return this.associate(attribute, name, definition.call(this, value));
                                                              });
                                                            });
                                                            g.behavior('method', g.behaviors.method);
                                                            for (var k in f.attributes) has(f.attributes, k) && g.associate(k, f.attributes[k], f[k])})},

              $c = merge(copy_of({behaviors: {method: function (v) {return bind(v, this)}}}).
                           behavior('ref', fn('$0')).behavior('shallow', shallow_copy).method('clone',     function (f) {return se(copy_of(this), f)}).
                                                                                       method('configure', function (f) {return se(this, f)}),
                         {deglobalize: function () {_global.caterwaul = _caterwaul; return this}}),
...
parse_associates_right = hash('= += -= *= /= %= &= ^= |= <<= >>= >>>= ~ ! new typeof u+ u- -- ++ u-- u++ ? if else function try catch finally for switch case with while do'),
   ...
   parse_r_until_block = annotate_keys({'function':2, 'if':1, 'do':1, 'catch':1, 'try':1, 'for':1, 'while':1}),
         parse_accepts = annotate_keys({'if':'else', 'do':'while', 'catch':'finally', 'try':'catch'}),
