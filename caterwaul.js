...
//   one. At this point the remaining element's 'l' and 'r' pointers will both be null, which means that it is in hierarchical form instead of linked form.
       ...
       syntax_node_inspect = fn('$0.inspect()'),
               syntax_node = $c.syntax_node = extend (fn('@data = $0, @length = 0, @l = @r = @parent = null'), {
               reparent: fn('@parent && @parent[0] === this && (@parent[0] = $0), @l = @r = null, this'),  fold_l: fn('@l && @append(@l.unlink(this)), this'),
                 append: fn('(this[@length++] = $0).parent = this'),                                       fold_r: fn('@r && @append(@r.unlink(this)), this'),
                 unlink: fn('@l && (@l.r = @r), @r && (@r.l = @l), @reparent($0)'),                       fold_lr: fn('@fold_l().fold_r()'),
                   wrap: fn('$0.l = @l, $0.r = @r, $0.parent = @parent, @reparent(@parent = $0)'),        fold_rr: fn('@fold_r().fold_r()'),
                sibling: fn('$0.parent = @parent, (@r = $0).l = this'),                                 append_to: fn('$0 && $0.append(this), this'),
...
//   what the parse_inverse_order table does: it maps operator names to parse_reduce_order subscripts. (e.g. 'new' -> 1.)
      ...
      for (var i = 0, l = ts.length, _ = null; _ = ts[i], i < l; ++i) _ == gs_top          ? (gs_top = grouping_stack.pop(),                head = head ? head.parent : parent, parent = null) :
...
//                           \                                               /  \                  <-- This can be done by saying _.l.wrap(new node('()')).parent.fold_r().
//                            bif <--> , <--> baz       -->               foo    (                     _.l.wrap() returns l again, .parent gets the wrapping node, and fold_r adds a child to it.
//                                                                                \
//                                                                                 bif <--> , <--> baz

      for (var i = 0, i0 = indexes[0], l = i0.length, _ = null, _d = null, _l = null; _ = i0[i], _d = _ && _.data, _l = _ && _.l, i < l; ++i)
        if (_d == '.')                                                                                                               _.fold_lr();
   else if (has(parse_ambiguous_group, _d) && _l && (_l.data == '.' || ! (has(lex_op, _l.data) || has(parse_not_a_value, _l.data)))) _l.wrap(new syntax_node(_d + parse_group[_d])).parent.fold_r();
