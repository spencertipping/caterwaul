...
//   This is where all of the logic comes together. The only remotely weird thing we do here is reverse both the pattern and expansion lists so that the macros get applied in the right order.
    ...
    identification_key = gensym(),
    identify = function (object) {return object[identification_key] || (object[identification_key] = gensym())},
    macro_expand_jit = function (t, patterns, expanders, context, compiled_function_cache) {
                         var last_length = -1, last_function = null;
                         var macroexpand_function = function () {
                           if (patterns.length === last_length) return last_function;
                           for (var k = null, ss = [], i = patterns.length - 1; i >= 0; --i) ss.push(patterns[i].inspect() + identify(expanders[i]));
                           if (compiled_function_cache[k = ss.join('#')]) return last_length = patterns.length, last_function = compiled_function_cache[k];
                           else {
