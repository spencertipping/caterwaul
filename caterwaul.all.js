...
//   This loads all of the production-use extensions.
    ...
    configuration('std', function () {this.configure('std.qs std.qg std.bind std.lvalue std.cond std.fn std.defmacro std.with_gensyms std.ref std.local std.compile_eval std.string')})});
...
// Generated by SDoc 
// Operator promotion module | Spencer Tipping
// Licensed under the terms of the MIT source code license
...
//   on. All sequences automatically get element caching and continuity -- that is, xs[i] will be set to xs.at(i) for you, and if you have xs[i] then you also have xs[i - 1] for i > 0.
    ...
    caterwaul.tconfiguration('std', 'seq.core', function () {
      ...
      this.seq.core.prototype.at(n)  = n < 0 ? this.finite_bound + n >= 0 ? this.at(this.finite_bound + n) : undefined : n < this.finite_bound ? this[n] : n >= this.length ? undefined :
...
//   There are a few different ways you can traverse a stream. The simplest way is to use the <se< operator, which stands for 'sequence each' -- this forces the stream and side-effectfully runs
//   an expression on each element; the original stream (now forced) is returned. You can do this for infinite streams, but you need a way to break out of the loop (e.g. with an escaping
//   continuation).

//   | console.log(x) <se< [1, 2, 3];                      // Logs 1, 2, 3 and returns a sequence constructed from [1, 2, 3]
//     (sa<< [1, 2, 3]).each(fn[x][console.log(x)]);       // Does the same thing

    tconfiguration('std iter', 'seq.class.traversal', function () {this.configure('seq.core').seq.traversal = {};
                                                                   this.seq.core.prototype.each(f) = (iter.n[i, this.length][f(this.at(i), i)], this)}).
    tconfiguration('std', 'seq.traversal', function () {
      let[ctransform = this.configure('seq.core seq.class.traversal').seq.traversal.each_comprehension_transform(body) =
                       qs[fn[x, i][_body, where*[t = this, $(n) = t.at(n), n = t.finite_bound]]].replace({_body: body})] in
...
//       (x + 1 <sa< [0]).map(fn[x][x * 2])        // ditto
      ...
      tconfiguration('std', 'seq.map', function () {
        let[ctransform = this.configure('seq.core seq.class.map').seq.map.map_comprehension_transform(body) =
                         qs[fn[x, i][_body, where*[t = this, $(n) = t.base.at(n), n = t.finite_bound]]].replace({_body: body}), map = this.seq.map] in
...
// Generally you won't want to use the above configurations; this one gathers those all into a coherent module.
  ...
  configuration('seq', function () {this.configure('seq.core.prepare seq.core.array seq.core.object seq.core.quantifiers seq.ana seq.traversal seq.map seq.filter seq.class.slice')});
