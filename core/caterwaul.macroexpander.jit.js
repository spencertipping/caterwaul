...
//       2. Each possibility in a comparison is equally likely. (I'm actually not assuming this in the final model; see below.)
...
//       4. Identifier nodes have much higher information density than operator nodes, and identifier nodes never have children. For concreteness' sake, let's assume that there are 100
//          identifiers and 20 operators.
...
//       2. We should perform comparisons in whichever order minimizes the expected number of future comparisons, under the assumption that each possibility is equally likely.
...
//     The weight (i.e. expected number of decisions) of this list of patterns is 1/20 right off the bat, since we assume that each operator is equally likely and there are about 20 operators. We
//     do the initial comparison, and given that the tree root is [] we now have two possibilities. If we compare the [0] child, then there are no more comparisons to perform regardless of the
//     outcome. The probability of choosing 'let' is 1/100, and the same is true of 'where'; so we weight the tree as 20/100 for let + 20/100 for where, meaning that we have a total weight of
//     1/20 * 40/100 = 1/50. (Note that the probabilities will actually be different; I'm just using 1/20 as an approximation.)

//     An interesting point comes up with respect to verification. Once we have narrowed the list down to just one pattern, we have to then make some additional checks to verify that the pattern
//     actually matches. (Mainly the arity checks I think.) This is also considered in the weighting. The reason is that it indicates something about the likelihood of a particular macro matching
//     a chunk of syntax. Macros with more complex patterns are statistically less likely to match. 
// Generated by SDoc 
