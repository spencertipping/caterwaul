...
//     potentially match the input, we have to proceed to the second if the first one rejects the match.
      ...
      partition_branch_template = parse('case _value: _statement; break'),
...
//     expensive and uncommon; a new array cons isn't going to make much difference at that point.
      ...
      generate_macroexpander_invocation = function (pattern_data, pattern, variables, paths) {
                                            return macroexpander_invocation_template.replace({_expander:             new ref(pattern_data[pattern.id()].expander),
                                                                                              _path_reference_array: generate_path_reference_array(variables, paths)})},

//     Pattern data.
//     We end up with lots of subarrays of the original pattern list. However, we need to be able to get back to the original expander for a given pattern, so we keep a hash of pattern data
//     indexed by the ID of the pattern tree.
...
//     anticipate that it would happen by accident. But it will cause erroneous macroexpansion results if you do.

      generate_pattern_sequence = function (pattern_data, patterns) {
// Generated by SDoc 
