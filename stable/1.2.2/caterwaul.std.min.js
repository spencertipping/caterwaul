...
}};return(parse_modifiers(match._thing,use(forms.normal,forms.inormal),use(forms.bang,forms.ibang),use(forms.tbang,forms.itbang)))}).call(this)}},handle_map_forms=operator_case({normal:map,bang:each,tbang:flatmap,itbang:iterate}),handle_filter_forms=operator_case({normal:filter,bang:filter_not,tbang:map_filter,itbang:imap_filter}),handle_fold_forms=operator_case({normal:foldl,bang:foldr,tbang:unfold,inormal:ifoldl,ibang:ifoldr,itbang:iunfold}),handle_kmap_forms=operator_case({normal:kmap,bang:keach}),handle_kfilter_forms=operator_case({normal:kfilter,bang:kfilter_not,tbang:kmap_filter}),handle_vmap_forms=operator_case({normal:vmap,bang:veach}),handle_vfilter_forms=operator_case({normal:vfilter,bang:vfilter_not,tbang:vmap_filter}),handle_exists_forms=operator_case({normal:exists,bang:not_exists,tbang:r_exists}),block=anon(qsu),block_with_variable=anon(qsv),block_with_init=anon(qsw),block_with_variable_and_init=anon(qsx),block_with_closure=anon(qsy),block_with_seq=anon(qsz),standard_names={_x:"x",_x0:"x0",_xi:"xi",_xl:"xl",_xs:"xs",_xr:"xr"},prefixed_names=function(p){return{_x:p,_x0:(""+(p)+"0"),_xi:(""+(p)+"i"),_xl:(""+(p)+"l"),_xs:(""+(p)+"s"),_xr:(""+(p)+"r")}
},function_promotion=anon(qs10),promote_function=function(f){return function_promotion.replace({_f:f})},closure_wrapper=anon(qs11),close_body=function(vars,f){return closure_wrapper.replace(vars).replace({_f:f})
},seq_pattern=anon(qs12),promote_seq=function(f){return seq_pattern.replace({_x:f})},parse_body=function(tree,expand,normal,init){return(function(){var in_sequence_context=function(f){return expand.call(expand,promote_seq(f))
...
}).call(this)},tbang_modifier=anon(qs13),bang_modifier=anon(qs14),parse_modifiers=function(tree,normal,bang,tbang){return(function(){var result=null;return(((result=tbang_modifier.match(tree))?tbang(result._x):(result=bang_modifier.match(tree))?bang(result._x):normal(tree)))
}).call(this)};return([rule(qs15,qs16),rule(qs17,concat),rule(qs18,qs19),rule(qs1a,qs1b),rule(qs1c,qs1d),rule(qs1e,qs1f),rule(qs1g,qs1h),rule(qs1i,qs1j),rule(qs1k,qs1l),rule(qs1m,qs1n),rule(qs1o,qs1p),rule(qs1q,qs1r),rule(qs1s,qs1t),rule(qs1u,qs1v),rule(qs1w,handle_filter_forms),rule(qs1x,handle_map_forms),rule(qs1y,handle_fold_forms),rule(qs1z,handle_exists_forms),rule(qs20,handle_kmap_forms),rule(qs21,handle_vmap_forms),rule(qs22,handle_kfilter_forms),rule(qs23,handle_vfilter_forms)])
