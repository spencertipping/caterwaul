...
(function (f){return f(f,(function (x){return function (){return  ++x}})(1))})(function (self,unique,undefined){var _caterwaul=typeof caterwaul==='undefined'?undefined:caterwaul;
...
var precompile=(function (){var nontrivial_function_pattern=parse('function (_) {_}'),trivial_function_pattern=parse('function () {_}'),nontrivial_function_gensym_template=parse('function (_args, _gensym) {_body}'),trivial_function_gensym_template=parse('function (_gensym) {_body}'),mark_nontrivial_function_macro=function (references){return function (args,body){var s=gensym(),result=nontrivial_function_gensym_template.replace({_args:args,_gensym:s,_body:annotate_functions_in(body,references)});
...
return '\''+ks.join(' ')+'\''},serialize_ref=function (ref){if (ref.value.constructor===syntax_node)return syntax_ref_template.replace({_string:syntax_ref_string(ref.value)});
...
else throw new Error('syntax ref value is not serializable: '+ref.value)},variables_for=function (tree,environment){var names=[],values=[];
for (var k in environment)if (own.call(environment,k))names.push(k),values.push(serialize_ref(environment[k]));
...
return names.length?new syntax_node(';',vars):closure_null_template},precompiled_closure=function (tree){return closure_template.replace({_vars:variables_for(tree),_value:tree})},precompiled_function=function (tree){return signal_already_compiled(precompiled_closure(tree))},substitute_precompiled=function (references){return function (args_or_gensym,gensym_or_body,body){var ref=references[args_or_gensym.data]||references[gensym_or_body.data];
return ref&&ref.compiled&&precompiled_function(ref.compiled)}},perform_substitution=function (references,tree){var expander=substitute_precompiled(references);
...
return compile(perform_substitution(references,references.annotated))}})();
...
return this}).field('syntax',syntax_node).field('ref',ref).field('parse',parse).field('compile',compile).field('gensym',gensym).field('map',map).field('self',self).field('unique',unique).field('macroexpansion',macroexpansion).field('replica',replica).field('configurable',configurable).field('caterwaul',caterwaul_core).field('decompile',parse).field('composition',composition).field('global',function (){return caterwaul_global}).field('precompiled_internal_table',{}).field('is_caterwaul',is_caterwaul).method('precompile',precompile).method('precompiled_internal',function (f){var k=gensym();
