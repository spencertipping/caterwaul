...
//     These functions let you modify nodes in-place. They're used during syntax folding and shouldn't really be used after that (hence the underscores).
      ...
      _prefix:   function (d) {return (this.prefix_data || (this.prefix_data = [])).push(d), this},
...
//     An accessor that populates the prefix_data field if necessary. This is useful when dealing with nodes that don't have any prefix data to start with (i.e. stuff that wasn't generated by the
//     parser). Temporary results are stored eagerly to reduce overall GC load -- the assumption is that this method will be called more than once.

      prefixes: function () {return this.prefix_data || (this.prefix_data = [])},
...
//     baz'. Even though this construct is unambiguous, most Javascript runtimes fail to accept it.
      ...
      serialize: function (xs, depth) {var l = this.length, d = this.data, d1 = depth - 1, push = function (x) {xs.push(x)};

                                       if (l && depth === 0) return push('...');

                                       push(this.prefix_data ? this.prefix_data.join('') : ' ');
...
//       avoid duplicated logic.) For reasons that are not entirely intuitive, the lexer sometimes produces the artifact 'u;'. This is never useful, so I have a case dedicated to removing it.
        ...
        if (t === false) prefixes.push(s.substring(mark, i));
