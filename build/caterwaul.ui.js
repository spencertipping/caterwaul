...
caterwaul.words(caterwaul.js())(function ($) {
  $.jquery(caterwaul_function) = caterwaul_function -se-
                                 it.modifiers.push(given.match in this(jquery_expand(anon_pattern.replace({_x: match._expression}))) -when [match._modifier.data === 'jquery'])

                                 -where [anon_pattern  = anon('J[_x]'),
                                         jquery_expand = $.expander(jquery_macros.concat(string_macros).concat(search_macros))],
...
//   quotes a compiled pattern.
  ...
  where [jq            = 'jQuery',
         ...
         rule(p, e)    = $.replacer(anon(p), e.constructor === Function ? this(e.call(this, match)) -given.match : anon(e)),

         hyphenate(s)  = s.replace(/_/g, '-'),

         p             = where [p_pattern = anon('P[_thing]')] in p_pattern.replace({_thing: node}) -given.node,

         jquery_macros = [rule('J[_element]',                 given.match [match._element.is_constant() || match._element.length ? wrap_in_jquery(match) :
