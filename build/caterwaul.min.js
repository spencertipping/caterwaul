...
}}})},as:function(aw){return this.data===aw?this:new this.constructor(aw).push(this)},bindings:function(ax){var aw=ax||{};this.reach(function(ay){ay.add_bindings_to(ax)
});return aw},expressions:function(ax){var aw=ax||{};this.reach(function(ay){ay.add_expressions_to(ax)});return aw},add_bindings_to:function(aw){},add_expressions_to:function(aw){},contains:function(az){var aw=az(this);
...
}else{if(F(at,aA)){return ay(aA),this[0].serialize(az),ay(at[aA])}else{if(F(aa,aA)){return ay("/* unary "+aA+" node */"),this[0].serialize(az)}else{return this[0].serialize(az),ay(aA)
}}}case 2:if(F(X,aA)){return this[0].serialize(az),ay(aA.charAt(0)),this[1].serialize(az),ay(aA.charAt(1))}else{if(F(m,aA)){return ay(aA),this[0].serialize(az),this[1].serialize(az)
}else{if(F(W,aA)){return this[0].serialize(az),this[1].serialize(az)}else{if(aA===";"){return this[0].serialize(az),ay(aw),this[1].serialize(az)}else{return this[0].serialize(az),ay(aA),this[1].serialize(az)
}}}}default:if(F(x,aA)){return this[0].serialize(az),ay(aA),this[1].serialize(az),ay(":"),this[2].serialize(az)}else{if(F(m,aA)){return this.accepts(this[2])&&!this[1].ends_with_block()?(ay(aA),this[0].serialize(az),this[1].serialize(az),ay(aw),this[2].serialize(az)):(ay(aA),this[0].serialize(az),this[1].serialize(az),this[2].serialize(az))
...
}else{this.e=ax,this.data=j(aw&&aw.constructor===String?aw:"e")}},H.ref_common,{add_expressions_to:function(aw){aw[this.data]=this.e}});H.opaque_tree=H.syntax_subclass(function(ax,aw){if(ax instanceof this.constructor){this.data=ax.data,this.expression_refs=ax.expression_refs
}else{this.data=ax.toString(),this.expression_refs=aw||ax.caterwaul_expression_ref_table}},{add_expressions_to:function(aw){this.expression_refs&&H.merge(aw,this.expression_refs)
},serialize:function(aw){return aw.push(this.data),aw},parse:function(){return H.parse(this.data)}});H.syntax=ah(H.syntax_subclass(function(aB){if(aB instanceof this.constructor){this.data=aB.data,this.length=0
}else{this.data=aB&&aB.toString();this.length=0;for(var az=1,aw=arguments.length,ay;ay=arguments[az],az<aw;++az){for(var ax=0,aD=ay.length,aA,aC;ay instanceof Array?(aA=ay[ax],ax<aD):(aA=ay,!ax);
++ax){this._append((aC=aA.constructor)===String||aC===Number||aC===Boolean?new this.constructor(aA):aA)}}}},H.javascript_tree_type_methods,H.javascript_tree_serialization_methods),function(){this.from_string=function(aw){return new H.syntax('"'+aw.replace(/"/g,'\\"').replace(/\n/g,"\\n")+'"')
...
try{return(new Function(aF,ax)).call(ay["this"],ay)}catch(aC){throw new Error((aC.message||aC)+" while compiling "+ax)}};var G=$.parse("new caterwaul.syntax(_data)"),t=$.parse("new caterwaul.syntax(_data, _xs)");
H.syntax_to_expression=function(ax){if(ax.length){for(var aw=new $.syntax(","),az=0,ay=ax.length;az<ay;++az){aw.push($.syntax_to_expression(ax[az]))}return t.replace({_data:H.syntax.from_string(ax.data),_xs:aw.unflatten()})
...
for(var ay in aw){if(ac.call(aw,ay)){aC.push(new H.syntax(ay)),aA.push(aw[ay]),aD[ay]=H.syntax_to_expression(aw[ay])}}var aB=H.gensym("result"),ax=aE.expression_ref_table?V.replace({_result:aB,_expression_ref_table:H.syntax.from_object(aD)}):caterwaul.empty;
return aC.length?C.replace({_bindings:aC.unflatten(),_expressions:aA.unflatten(),_result_init:ax,_body:aF}):aF};H.gensym_rename_table=function(aE){var aD={},aB=[];
