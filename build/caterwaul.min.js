...
}),function(){this.data=r}):this.padding_encase(r):this.map(function(n){return n.replace(rs)})},thin_clone:function(){return this.map(function(){return false})},repopulated_with:function(xs){return new this.constructor(this.data,xs)
...
},suffix:function(d){return this.suffixes().push(d),this},suffixes:function(){return this.suffix_data||(this.suffix_data=[])},padding_encase:function(x){return new caterwaul_global.padded_tree(this.prefix_data,this.suffix_data,x)
...
});return this_p>p?result.as("("):result},serialize:function(xs,depth){var l=this.length,d=this.data,d1=depth-1,e=function(x){x&&xs.push(x)},p=this.prefix_data?this.prefix_data.join(""):" ",i=this.infix_data&&this.infix_data.join(""),s=this.suffix_data&&this.suffix_data.join("");
if(depth===0&&(l||d.length>32)){return e("...")}switch(l){case 0:if(has(parse_r_optional,d)){return e(p),e(d.replace(/^u/,"")),e(s)}else{if(has(parse_group,d)){return e(p),e(d),e(i),e(parse_group[d]),e(s)
...
}else{return this[0].serialize(xs,d1),e(p),e(d),this[1].serialize(xs,d1),e(s)}}}default:if(has(parse_ternary,d)){return this[0].serialize(xs,d1),e(p),e(d),this[1].precedence()>this.precedence()?(this[1].as("(").serialize(xs,d1),e(i),e(":"),this[2].serialize(xs,d1),e(s)):(this[1].serialize(xs,d1),e(i),e(":"),this[2].serialize(xs,d1),e(s))
...
}},caterwaul_global.ref_common);caterwaul_global.padded_tree=caterwaul_global.syntax_subclass(function(p,s,t){if(p instanceof this.constructor){this.length=0,this.prefix_data=p.prefix_data,this.suffix_data=p.suffix_data
}else{this.length=1,this[0]=t,this.prefix_data=p,this.suffix_data=s}},caterwaul_global.ref_common,{data:"@padded",replace:function(rs){return new this.constructor(this.prefix_data,this.suffix_data,this[0].replace(rs))
},serialize:function(xs,depth){this.prefix_data&&xs.push(this.prefix_data.join(""));this[0].serialize(xs,depth);this.suffix_data&&xs.push(this.suffix_data.join(""))
