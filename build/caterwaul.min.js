...
prefix=[];return k};if(l===0){return empty}while((mark=i)<l){while(lex_space[c=cs(i)]&&i++<l){}prefix.push(s.substring(mark,i)),mark=i;esc=exp=dot=t=false;if(lex_bracket[c]){t=!!++i;
...
}if(t===false){prefix.push(s.substring(mark,i));continue}t=t===true?s.substring(mark,i):t==="u;"?";":t;t===gs_top?(grouping_stack.pop(),gs_top=grouping_stack[grouping_stack.length-1],(head||parent).infix_data=shift_prefix(),head=head?head.p:parent,parent=null):(has(parse_group,t)?(grouping_stack.push(gs_top=parse_group[t]),parent=push(new_node(new syntax_node(t))),groups.push(parent),head=null):push(new_node(new syntax_node(t))),has(parse_inverse_order,t)&&indexes[parse_inverse_order[t]].push(head||parent));
