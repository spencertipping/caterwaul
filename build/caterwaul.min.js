...
},_unlink:function(n){return this.l&&(this.l.r=this.r),this.r&&(this.r.l=this.l),delete this.l,delete this.r,this._reparent(n)},_prefix:function(d){return(this.prefix_data||(this.prefix_data=[])).push(d),this
},pop:function(){return --this.length,this},push:function(x){return this[this.length++]=caterwaul_global.syntax.promote(x||empty),this},id:function(){var id=gensym("id");
return(this.id=function(){return id})()},is_caterwaul_syntax:true,each:function(f){for(var i=0,l=this.length;i<l;++i){f(this[i],i)}return this},map:function(f){for(var n=new this.constructor(this),i=0,l=this.length;
...
});return result},add_bindings_to:function(hash){},add_expressions_to:function(hash){},resolve:function(){return this},reduce:function(){return this},prefixes:function(){return this.prefix_data||(this.prefix_data=[])
},contains:function(f){var result=f(this);if(result){return result}for(var i=0,l=this.length;i<l;++i){if(result=this[i].contains(f)){return result}}},match:function(target,variables){target=target.constructor===String?caterwaul_global.parse(target):target;
...
});return this_p>p?result.as("("):result},serialize:function(xs,depth){var l=this.length,d=this.data,d1=depth-1,push=function(x){xs.push(x)};if(l&&depth===0){return push("...")
}push(this.prefix_data?this.prefix_data.join(""):" ");switch(l){case 0:if(has(parse_r_optional,d)){return push(d.replace(/^u/,""))}else{if(has(parse_group,d)){return push(d),push(parse_group[d])
}else{return push(d)}}case 1:if(has(parse_r,d)||has(parse_r_optional,d)){return push(d.replace(/^u/,"")),this[0].serialize(xs,d1)}else{if(has(parse_misleading_postfix,d)){return this[0].serialize(xs,d1),push(d)
}else{if(has(parse_group,d)){return push(d),this[0].serialize(xs,d1),push(parse_group[d])}else{if(has(parse_lr,d)){return this[0].serialize(xs,d1)}else{return this[0].serialize(xs,d1),push(d)
}}}}case 2:if(has(parse_invocation,d)){return this[0].serialize(xs,d1),push(d.charAt(0)),this[1].serialize(xs,d1),push(d.charAt(1))}else{if(has(parse_r_until_block,d)){return push(d),this[0].serialize(xs,d1),this[1].serialize(xs,d1)
}else{if(has(parse_invisible,d)){return this[0].serialize(xs,d1),this[1].serialize(xs,d1)}else{if(d===";"){return this[0].serialize(xs,d1),push(";\n"),this[1].serialize(xs,d1)
}else{return this[0].serialize(xs,d1),push(d),this[1].serialize(xs,d1)}}}}default:if(has(parse_ternary,d)){return this[0].serialize(xs,d1),push(d),this[1].precedence()>this.precedence()?(this[1].as("(").serialize(xs,d1),push("\n:"),this[2].serialize(xs,d1)):(this[1].serialize(xs,d1),push("\n:"),this[2].serialize(xs,d1))
}else{if(has(parse_r_until_block,d)){return this.accepts(this[2])&&!this[1].ends_with_block()?(push(d),this[0].serialize(xs,d1),this[1].serialize(xs,d1),push(";\n"),this[2].serialize(xs,d1)):(push(d),this[0].serialize(xs,d1),this[1].serialize(xs,d1),this[2].serialize(xs,d1))
}else{return this.unflatten().serialize(xs,d1)}}}}};caterwaul_global.ref_common=caterwaul_global.merge({},caterwaul_global.javascript_tree_type_methods,caterwaul_global.javascript_tree_metadata_methods,caterwaul_global.javascript_tree_serialization_methods,{replace:function(replacements){var r;
...
},grouping_stack=[],gs_top=null,head=null,parent=null,indexes=map(function(){return[]},parse_reduce_order),invocation_nodes=[],all_nodes=[empty],new_node=function(n){return all_nodes.push(n),n.prefix_data=prefix,prefix=[],n
},push=function(n){return head?head._sibling(head=n):(head=n._append_to(parent)),new_node(n)},syntax_node=this.syntax,groups=[],ternaries=[],prefix=[];if(l===0){return empty
}while((mark=i)<l){while(lex_space[c=cs(i)]&&i<l){++i}prefix.push(s.substring(mark,mark=i));esc=exp=dot=t=false;if(lex_bracket[c]){t=!!++i;re=lex_opener[c]}else{if(c===lex_slash&&cs(i+1)===lex_star&&(i+=2)){while(++i<l&&cs(i)!==lex_slash||cs(i-1)!==lex_star){}t=!++i
...
}if(t===false){prefixes.push(s.substring(mark,i))}t=t===true?s.substring(mark,i):t==="u;"?";":t;t===gs_top?(grouping_stack.pop(),gs_top=grouping_stack[grouping_stack.length-1],head=head?head.p:parent,parent=null):(has(parse_group,t)?(grouping_stack.push(gs_top=parse_group[t]),parent=push(new_node(new syntax_node(t))),groups.push(parent),head=null):push(new_node(new syntax_node(t))),has(parse_inverse_order,t)&&indexes[parse_inverse_order[t]].push(head||parent));
