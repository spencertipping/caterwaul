...
}o[max_length_key]=max;return o},has=function(o,p){return p!=null&&!(p.length>o[max_length_key])&&own.call(o,p)},own=Object.prototype.hasOwnProperty,caterwaul_global=caterwaul.merge(caterwaul,{map:map,rmap:rmap,gensym:gensym,is_gensym:is_gensym}),lex_op=hash(". new ++ -- u++ u-- u+ u- typeof void u~ u! ! * / % + - << >> >>> < > <= >= instanceof in == != === !== & ^ | && || ? = += -= *= /= %= &= |= ^= <<= >>= >>>= : , return throw case var const break continue else u; ;"),lex_table=function(s){for(var i=0,xs=[false];
...
i<l;++i){n.push(f(this[i],i)||this[i])}return n},reach:function(f){f(this);this.each(function(n){n.reach(f)});return this},rmap:function(f){var r=f(this);return !r||r===this?this.map(function(n){return n.rmap(f)
}):r===true?this:r.rmap===undefined?new this.constructor(r):r},peach:function(f){this.each(function(n){n.peach(f)});f(this);return this},pmap:function(f){var t=this.map(function(n){return n.pmap(f)
});return f(t)},clone:function(){return this.rmap(function(){return false})},collect:function(p){var ns=[];this.reach(function(n){p(n)&&ns.push(n)});return ns},replace:function(rs){var r;
return own.call(rs,this.data)&&(r=rs[this.data])?r.constructor===String?se(this.map(function(n){return n.replace(rs)}),function(){this.data=r}):r:this.map(function(n){return n.replace(rs)
})},repopulated_with:function(xs){return new this.constructor(this.data,xs)},with_data:function(d){return new this.constructor(d,Array.prototype.slice.call(this))
...
});return result},add_bindings_to:function(hash){},add_expressions_to:function(hash){},resolve:function(){return this},contains:function(f){var result=f(this);if(result){return result
}for(var i=0,l=this.length;i<l;++i){if(result=this[i].contains(f)){return result}}},match:function(target,variables){target=target.constructor===String?caterwaul_global.parse(target):target;
...
},this)).join(" ")+")"}else{return this.data}}};caterwaul_global.syntax_subclasses=[];caterwaul_global.syntax_subclass=function(ctor){var extensions=Array.prototype.slice.call(arguments,1),proxy=function(){return ctor.apply(this,arguments)
};caterwaul_global.merge.apply(this,[proxy.prototype,syntax_common].concat(extensions));caterwaul_global.syntax_subclasses.push(proxy);proxy.prototype.constructor=proxy;
return proxy};caterwaul_global.syntax_extend=function(){for(var i=0,l=caterwaul_global.syntax_subclasses.length,es=Array.prototype.slice.call(arguments);i<l;++i){caterwaul_global.merge.apply(this,[caterwaul_global.syntax_subclasses[i].prototype].concat(es))
}caterwaul_global.merge.apply(this,[syntax_common].concat(es));return caterwaul_global};var parse_hex=caterwaul_global.parse_hex=function(digits){for(var result=0,i=0,l=digits.length,d;
...
},guarded:function(p){var this_p=this.is_group()||this.is_associative()?undefined:this.precedence(),right=this.is_right_associative(),result=this.map(function(x,i){return x.guarded(this_p-(!right&&!!i))
});return this_p>p?result.as("("):result},serialize:function(xs){var l=this.length,d=this.data,semi=";\n",push=function(x){if(lex_ident[xs[xs.length-1].charCodeAt(0)]===lex_ident[x.charCodeAt(0)]){xs.push(" ",x)
...
}}case 1:if(has(parse_r,d)||has(parse_r_optional,d)){return push(d.replace(/^u/,"")),this[0].serialize(xs)}else{if(has(parse_misleading_postfix,d)){return this[0].serialize(xs),push(d)
}else{if(has(parse_group,d)){return push(d),this[0].serialize(xs),push(parse_group[d])}else{if(has(parse_lr,d)){return this[0].serialize(xs)}else{return this[0].serialize(xs),push(d)
}}}}case 2:if(has(parse_invocation,d)){return this[0].serialize(xs),push(d.charAt(0)),this[1].serialize(xs),push(d.charAt(1))}else{if(has(parse_r_until_block,d)){return push(d),this[0].serialize(xs),this[1].serialize(xs)
}else{if(has(parse_invisible,d)){return this[0].serialize(xs),this[1].serialize(xs)}else{if(d===";"){return this[0].serialize(xs),push(semi),this[1].serialize(xs)
}else{return this[0].serialize(xs),push(d),this[1].serialize(xs)}}}}default:if(has(parse_ternary,d)){return this[0].serialize(xs),push(d),this[1].serialize(xs),push(":"),this[2].serialize(xs)
...
}else{return this.unflatten().serialize(xs)}}}}};caterwaul_global.ref_common=caterwaul_global.merge({},caterwaul_global.javascript_tree_type_methods,caterwaul_global.javascript_tree_metadata_methods,caterwaul_global.javascript_tree_serialization_methods,{replace:function(replacements){var r;
...
}},{resolve:function(){return this.expression_refs?caterwaul_global.late_bound_tree(new this.constructor(this.data),this.expression_refs):this},serialize:function(xs){return xs.push(this.data),xs
...
},push=function(n){return head?head._sibling(head=n):(head=n._append_to(parent)),new_node(n)},syntax_node=this.syntax,ternaries=[];if(l===0){return empty}while((mark=i)<l){while(lex_space[c=cs(i)]&&i<l){mark=++i
}esc=exp=dot=t=false;if(lex_bracket[c]){t=!!++i;re=lex_opener[c]}else{if(c===lex_slash&&cs(i+1)===lex_star&&(i+=2)){while(++i<l&&cs(i)!==lex_slash||cs(i-1)!==lex_star){}t=!++i
}else{if(c===lex_slash&&cs(i+1)===lex_slash){while(++i<l&&!lex_eol[cs(i)]){}t=false}else{if(c===lex_hash){while(++i<l&&!lex_eol[cs(i)]){}t=false}else{if(lex_quote[c]&&(close=c)&&re&&!(re=!(t=s.charAt(i)))){while(++i<l&&(c=cs(i))!==close||esc){esc=!esc&&c===lex_back
