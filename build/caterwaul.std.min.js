...
}else{return this.modifiers.hasOwnProperty(modifier=regular_match._modifier.data)&&this.modifiers[modifier].call(this,regular_match)}}};var each_node=function(node){var p=(function(xs1){var x,x0,xi,xl,xr;
...
for(var x=xs1[0],xi=0,xl=xs1.length,x1;xi<xl;++xi){x=xs1[xi];if(x1=(/^#/.test(x))){return x1}}return false}).call(this,node.suffixes());(p||i||s)&&(node=node.thin_clone());
p&&(node.prefix_data=(function(xs1){var x,x0,xi,xl,xr;for(var xr=new xs1.constructor(),xi=0,xl=xs1.length;xi<xl;++xi){x=xs1[xi],(/^#/.test(x))||xr.push(x)}return xr
}).call(this,node.prefix_data));i&&(node.infix_data=(function(xs1){var x,x0,xi,xl,xr;for(var xr=new xs1.constructor(),xi=0,xl=xs1.length;xi<xl;++xi){x=xs1[xi],(/^#/.test(x))||xr.push(x)
}return xr}).call(this,node.infix_data));s&&(node.suffix_data=(function(xs1){var x,x0,xi,xl,xr;for(var xr=new xs1.constructor(),xi=0,xl=xs1.length;xi<xl;++xi){x=xs1[xi],(/^#/.test(x))||xr.push(x)
}return xr}).call(this,node.suffix_data));return string_interpolator.call(this,node)||literal_modifier.call(this,node)||node.length&&(modifier.call(this,node)||function_destructure.call(this,node)||tuple_destructure.call(this,node)||infix_function.call(this,node)||infix_method.call(this,node)||postfix_function.call(this,node))
