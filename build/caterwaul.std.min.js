...
return((function(){var unrecognized=function(reason){return(function(){throw new Error(reason)}).call(this)},use_form=function(form,xs,body,init,vars){return form?vars?form.replace($.merge({_s:xs,_f:body,_init:init},vars)):form:unrecognized(("unsupported sequence operator or modifiers used on "+(body)+""))
...
}},iform_function=function(form){return function(body,init,vars){return use_form(form,xs,body,init,vars)}},use_x0=function(form){return form&&(function(it){return(it&&it._x0.data==="_x0")
}).call(this,(qsu.match(form)))},use=function(form,iform){return function(body){return parse_body(body,expander,use_x0(form),use_x0(iform),form_function(form),iform_function(iform))
}};return(parse_modifiers(match._thing,use(forms.normal,forms.inormal),use(forms.bang,forms.ibang),use(forms.tbang,forms.itbang)))}).call(this)}},handle_map_forms=operator_case({normal:map,bang:each,tbang:flatmap,itbang:iterate}),handle_filter_forms=operator_case({normal:filter,bang:filter_not,tbang:map_filter,itbang:imap_filter}),handle_fold_forms=operator_case({normal:foldl,bang:foldr,tbang:unfold,inormal:ifoldl,ibang:ifoldr,itbang:iunfold}),handle_kmap_forms=operator_case({normal:kmap,bang:keach}),handle_kfilter_forms=operator_case({normal:kfilter,bang:kfilter_not,tbang:kmap_filter}),handle_vmap_forms=operator_case({normal:vmap,bang:veach}),handle_vfilter_forms=operator_case({normal:vfilter,bang:vfilter_not,tbang:vmap_filter}),handle_exists_forms=operator_case({normal:exists,bang:not_exists,tbang:r_exists}),block=anon(qsv),vblock=anon(qsw),iblock=anon(qsx),ivblock=anon(qsy),block_with_closure=anon(qsz),block_with_seq=anon(qs10),standard_names={_x:"x",_x0:"x0",_xi:"xi",_xl:"xl",_xs:"xs",_xr:"xr"},prefixed_names=function(p){return{_x:p,_x0:(""+(p)+"0"),_xi:(""+(p)+"i"),_xl:(""+(p)+"l"),_xs:(""+(p)+"s"),_xr:(""+(p)+"r")}
},function_promotion=anon(qs11),function_promotion_x0=anon(qs12),promote_function=function(vars,f,use_x0){return(use_x0?function_promotion_x0:function_promotion).replace($.merge({_f:f},vars))
},closure_wrapper=anon(qs13),close_body=function(vars,f){return closure_wrapper.replace($.merge({_f:f},vars))},seq_pattern=anon(qs14),promote_seq=function(f){return seq_pattern.replace({_x:f})
},parse_body=function(tree,expand,nx0,ix0,normal,init){return(function(){var in_sequence_context=function(f){return expand.call(expand,promote_seq(f))},sequence_context_normal=function(f,names){return normal(in_sequence_context(f),names)
},sequence_context_init=function(f,init_expression,names){return init(in_sequence_context(f),init_expression,names)},wrapping_normal=function(f,names){return normal(close_body(names,f),names)
},wrapping_init=function(f,init_expression,names){return init(close_body(names,f),init_expression,names)},r=null;return(((r=block_with_seq.match(tree))?parse_body(r._x,expand,sequence_context_normal,sequence_context_init):(r=block_with_closure.match(tree))?parse_body(r._x,expand,wrapping_normal,wrapping_init):(r=ivblock.match(tree))?init(r._x,r._init,prefixed_names(r._var)):(r=iblock.match(tree))?init(r._x,r._init,standard_names):(r=vblock.match(tree))?normal(r._x,prefixed_names(r._var)):(r=block.match(tree))?normal(r._x,standard_names):normal(promote_function(standard_names,tree,nx0||ix0),null)))
}).call(this)},tbang_modifier=anon(qs15),bang_modifier=anon(qs16),parse_modifiers=function(tree,normal,bang,tbang){return(function(){var result=null;return(((result=tbang_modifier.match(tree))?tbang(result._x):(result=bang_modifier.match(tree))?bang(result._x):normal(tree)))
}).call(this)};return([rule(qs17,qs18),rule(qs19,concat),rule(qs1a,qs1b),rule(qs1c,qs1d),rule(qs1e,qs1f),rule(qs1g,qs1h),rule(qs1i,qs1j),rule(qs1k,qs1l),rule(qs1m,qs1n),rule(qs1o,qs1p),rule(qs1q,qs1r),rule(qs1s,qs1t),rule(qs1u,qs1v),rule(qs1w,qs1x),rule(qs1y,handle_filter_forms),rule(qs1z,handle_map_forms),rule(qs20,handle_fold_forms),rule(qs21,handle_exists_forms),rule(qs22,handle_kmap_forms),rule(qs23,handle_vmap_forms),rule(qs24,handle_kfilter_forms),rule(qs25,handle_vfilter_forms)])
