 (function($,mark1) {var syntax_manipulator=function(base_case,mark2) {var result=function(x,mark3) {if(x.constructor===Array) {for(var i=0,l=x.length,ys= [ ] ;
i<l;
 ++i)ys.push(result(x[i] ) ) ;
return function(tree,mark4) {for(var i=ys.length-1,r;
i>=0;
...
return result} ;
$.pattern=syntax_manipulator(function(pattern,mark5) {return function(tree,mark6) {return pattern.match(tree) } } ) ;
$.expander=syntax_manipulator(function(expander,mark7) {return function(match,mark8) {return expander.replace(match) } } ) ;
$.alternatives=syntax_manipulator(function(alternative,mark9) {throw new Error( 'must use replacer functions with caterwaul.alternatives()' ) } ) ;
$.reexpander=function(expander,marka) {var e=$.expander(expander) ;
return function(match,markb) {var r=e.call(this,match) ;
return r&&this(r) } } ;
var composer=function(expander_base_case,markc) {return function(pattern,expander,markd) {var new_pattern=$.pattern(pattern) ,new_expander=expander_base_case(expander) ;
return function(tree,marke) {var match=new_pattern.call(this,tree) ;
return match&&new_expander.call(this,match) } } } ;
$.replacer=composer($.expander) ;
$.rereplacer=composer($.reexpander) ;
$.macroexpand=function(tree,markf) {return $($.alternatives(Array.prototype.slice.call(arguments,1) ) ) (tree) } } ) (caterwaul) ;
 (function($,markg) {$.anonymizer=function(markh) {for(var translation_table= { } ,i=0,l=arguments.length;
i<l;
 ++i)translation_table[arguments[i] ] =$.gensym(arguments[i] ) ;
return function(node,marki) {return $.parse(node) .replace(translation_table) } } } ) (caterwaul) ;
 (function($,markj) {$.js=function(macroexpander,markk) {var string_interpolator=function(node,markl) {var s=node.data,q=s.charAt(0) ,syntax=$.syntax;
...
var function_rule=$.rereplacer( '_left(_args) = _right' , '_left = (function (_args) {return _right})' ) ,function_args_rule=$.rereplacer( '_left(_var = arguments) = _right' , '_left = (function () {var _var = arguments; return _right})' ) ,function_destructure=function(node,markm) {return function_args_rule.call(this,node) ||function_rule.call(this,node) } ;
var bracket_modifier_form=$.pattern( '_modifier[_expression]' ) ,slash_modifier_form=$.pattern( '_expression /_modifier' ) ,minus_modifier_form=$.pattern( '_expression -_modifier' ) ,in_modifier_form=$.pattern( '_modifier in _expression' ) ,pipe_modifier_form=$.pattern( '_expression |_modifier' ) ,comma_modifier_form=$.pattern( '_expression, _modifier' ) ,dot_parameters=$.pattern( '_modifier._parameters' ) ,bracket_parameters=$.pattern( '_modifier[_parameters]' ) ,parameterized_wickets=$.pattern( '_expression <_modifier> _parameters' ) ,parameterized_minus=$.pattern( '_expression -_modifier- _parameters' ) ,modifier=function(node,markn) {var parameterized_match=parameterized_wickets.call(this,node) ||parameterized_minus.call(this,node) ;
...
var result=macroexpander?$(function(node,marko) {return macroexpander.call(this,node) ||string_interpolator.call(this,node) ||node.length&& (modifier.call(this,node) ||function_destructure.call(this,node) ) } ) :$(function(node,markp) {return string_interpolator.call(this,node) ||node.length&& (modifier.call(this,node) ||function_destructure.call(this,node) ) } ) ;
result.modifiers= [ ] ;
result.parameterized_modifiers= [ ] ;
return result} } ) (caterwaul) ;
 (function($,markq) {$.words=function(caterwaul_function,markr) {var filtered_expander=function(word,expander,marks) {return function(match,markt) {return match._modifier.data===word&&expander.call(this,match) } } ,modifier=function(word,expander,marku) {caterwaul_function.modifiers.push(filtered_expander(word,expander) ) } ,parameterized_modifier=function(word,expander,markv) {caterwaul_function.parameterized_modifiers.push(filtered_expander(word,expander) ) } ;
modifier( 'qs' ,function(match,markw) {return new $.ref(match._expression, 'qs' ) } ) ;
modifier( 'qse' ,function(match,markx) {return new $.ref(this(match._expression) , 'qse' ) } ) ;
modifier( 'reexpand' ,function(match,marky) {return this(this(match._expression) ) } ) ;
modifier( 'noexpand' ,function(match,markz) {return match._expression} ) ;
