...
Version 1.3.1 adds comment conversion. This is important; waul allows #-style comments to support shebang lines, but these are illegal in Javascript. To fix this, we convert hash comments to
regular ones here. This conversion happens nondestructively.

      var each_node = function (node) {if (node.prefix_data && node.prefix_data |/^#/.test |seq) node = node.clone() -se- node.prefix_data *[x.replace(/^#/, '//')] *!it.prefix /seq;
