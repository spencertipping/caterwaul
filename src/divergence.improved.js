...
//   the low 256 characters are indexed. High characters will trigger sparse arrays, which may degrade performance.
          ...
          lex_table = function (s) {for (var i = 0, xs = [false]; i < 8; ++i) xs = xs.concat(xs); for (var i = 0, l = s.length; i < l; ++i) xs[s.charCodeAt(i)] = true; return xs},
             lex_op = hash('. new ++ -- u+ u- typeof u~ u! * / % + - << >> >>> < > <= >= instanceof in == != === !== & ^ | && || ? = += -= *= /= %= &= |= ^= <<= >>= >>>= : ,'),
...
else if               (c === lex_slash && re && ! (re = ! (t = '/'))) {while                              (++i < l && (c = cs[i]) !== lex_slash || esc)  esc = ! esc && c === lex_back;
                                                                       while                                       (++i < l && ex_regexp_suffix[cs[i]]);                                t = true}
else if (lex_quote[c] && (close = c) && ! (re = ! (t = s.charAt(i)))) {while                                  (++i < l && (c = cs[i]) !== close || esc)  esc = ! esc && c === lex_back; t = !! ++i}
else if              (lex_punct[c] && (t = re ? 'u' : '', re = true))  while                (i < l && lex_punct[cs[i]] && has(lex_op, t + s.charAt(i)))  t += s.charAt(i++);
else if                              (lex_digit[c] && ! (re = false)) {while (++i < l && lex_digit[c = cs[i]] && ! (exp && (exp |= lex_exp[c] && ++i)));                                t = true}
else                                                                  {while                                          (++i < l && lex_ident[c = cs[i]]); re = has(lex_op, t = s.substring(mark, i))}
