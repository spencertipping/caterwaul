...
//     baz'. Even though this construct is unambiguous, most Javascript runtimes fail to accept it.
      ...
      serialize: function (xs, depth) {var l = this.length, d = this.data, d1 = depth - 1, e = function (x) {xs.push(x)}, p = this.prefix_data ? this.prefix_data.join('') : ' ',
                           ...
                           switch (l) {case 0: if (has(parse_r_optional, d)) return e(p), e(d.replace(/^u/, ''));
                                          else if (has(parse_group, d))      return e(p), e(d), e(parse_group[d]);
                                          else                               return e(p), e(d);

                                       case 1: if (has(parse_r, d) || has(parse_r_optional, d)) return e(p), e(d.replace(/^u/, '')), this[0].serialize(xs, d1), e(s);
                                          else if (has(parse_misleading_postfix, d))            return this[0].serialize(xs, d1), e(p), e(d), e(s);
                                          else if (has(parse_group, d))                         return e(p), e(d), this[0].serialize(xs, d1), e(s), e(parse_group[d]);
                                          else if (has(parse_lr, d))                            return e(p), this[0].serialize(xs, d1), e(s);
                                          else                                                  return this[0].serialize(xs, d1), e(p), e(d), e(s);

                                       case 2: if (has(parse_invocation, d))    return this[0].serialize(xs, d1), e(p), e(d.charAt(0)), this[1].serialize(xs, d1), e(s), e(d.charAt(1));
                                          else if (has(parse_r_until_block, d)) return e(p), e(d), this[0].serialize(xs, d1), this[1].serialize(xs, d1), e(s);
                                          else if (has(parse_invisible, d))     return this[0].serialize(xs, d1), this[1].serialize(xs, d1), e(s);
                                          else if (d === ';')                   return this[0].serialize(xs, d1), e(p), e(';'), this[1].serialize(xs, d1), e(s);
                                          else                                  return this[0].serialize(xs, d1), e(p), e(d), this[1].serialize(xs, d1), e(s);

                                      default: if (has(parse_ternary, d))       return this[0].serialize(xs, d1), e(p), e(d), this[1].precedence() > this.precedence()
                                                                                  ? (this[1].as('(').serialize(xs, d1), e(s), e(':'), this[2].serialize(xs, d1))
                                                                                  : (this[1].        serialize(xs, d1), e(s), e(':'), this[2].serialize(xs, d1));
...
//     node; in this case we append the node as 'head'. Another case is when 'head' exists; in that case we update head to be the new node, which gets added as a sibling of the old head.
        ...
        var s = input.toString().replace(/^\s*|\s*$/g, ''), mark = 0, c = 0, re = true, esc = false, dot = false, exp = false, close = 0, t = '', i = 0, l = s.length,
            ...
            invocation_nodes = [], all_nodes = [empty], new_node = function (n) {return all_nodes.push(n), n.prefix_data = prefix, prefix = [], n},
            push = function (n) {return head ? head._sibling(head = n) : (head = n._append_to(parent)), new_node(n)}, syntax_node = this.syntax, groups = [], ternaries = [], prefix = [];
...
//     position (we'll be incrementing i as we read characters), munch whitespace, and reset flags.
        ...
        while ((mark = i) < l) {
          ...
          prefix.push(s.substring(mark, mark = i));
          esc = exp = dot = t = false;
...
//       avoid duplicated logic.) For reasons that are not entirely intuitive, the lexer sometimes produces the artifact 'u;'. This is never useful, so I have a case dedicated to removing it.
        ...
        if (t === false) prefix.push(s.substring(mark, i));
...
//       write destructuring binds against syntax trees.
        ...
        t === gs_top ? (grouping_stack.pop(), gs_top = grouping_stack[grouping_stack.length - 1], head.suffix_data = prefix, prefix = [], head = head ? head.p : parent, parent = null) :
...
//     like this, we preserve the whitespace by assigning it as the suffix of the last-parsed node.

        if (prefix.length) all_nodes[i - 1].suffix_data = prefix;
