...
//     'using' modifier.
      ...
      bindings:    function (hash) {var result = hash || {}; this.reach(function (n) {n.add_bindings_to(hash)});    return result},
      expressions: function (hash) {var result = hash || {}; this.reach(function (n) {n.add_expressions_to(hash)}); return result},
...
//   This is Caterwaul's standard way of encoding reconstructible expression references. It should be a table of this form:
  ...
  // | {ref_name_1: syntax_node, ref_name_2: syntax_node, ...}

  // Each syntax node represents the expression that is used to construct the expression that ends up being bound. So, for example:

  // | > expression = caterwaul.parse('3 + 4')
//     > expression_ref = new caterwaul.expression_ref(expression)
//     > f = caterwaul.compile(caterwaul.parse('function () {return _foo}').replace({_foo: }))
//     > f.caterwaul_expression_ref_table
//     { e_a_gensym: expression }
    ...
    caterwaul_global.syntax = se(caterwaul_global.syntax_subclass(
                                 ...
                                 function () {this.from_string = function (s)  {return new caterwaul_global.syntax('"' + s.replace(/"/g, '\\"').replace(/\n/g, '\\n') + '"')};
                                              this.from_array  = function (xs) {for (var i = 0, c = new caterwaul_global.syntax(','), l = xs.length; i < l; ++i) c.push(xs[i]);
                                                                                return new caterwaul_global.syntax('[', c.unflatten())};

                                              this.from_object = function (o)  {var comma = new caterwaul_global.syntax(',');
                                                                                for (var k in o) if (own.call(o, k))
                                                                                  comma.push(new caterwaul_global.syntax(':', caterwaul_global.syntax.from_string(k), o[k].as('(')));
                                                                                return new caterwaul_global.syntax('{', comma.unflatten())}});
...
//   references. The consequence of this is that you won't be able to reconstruct a value that comes out of this function after precompilation. Generally you'll want to leave it set to true.
    ...
    var trivial_node_template    = $.parse('new caterwaul.syntax(_data)'),
        nontrivial_node_template = $.parse('new caterwaul.syntax(_data, _xs)');

    caterwaul_global.syntax_to_expression = function (tree) {
      if (tree.length) {for (var comma = new $.syntax(','), i = 0, l = tree.length; i < l; ++i) comma.push($.syntax_to_expression(tree[i]));
    ...
    caterwaul_global.late_bound_tree = function (tree, environment, options) {
      ...
      for (var k in bindings) if (own.call(bindings, k)) variables.push(new caterwaul_global.syntax(k)), expressions.push(bindings[k]),
                                                         table[k] = caterwaul_global.syntax_to_expression(bindings[k]);
