function (gensym_2_gmjc65nf_gy8ej6){(function (f){return f(f,(function (x){return function (){return  ++x}})(1))})(function (self,unique,undefined){var _caterwaul=typeof caterwaul==='undefined'?undefined:caterwaul;
var qw=function (x){return x.split(/\s+/)},id=function (x){return x},se=function (x,f){return f&&f.call(x,x)||x},genval=(function (n,m,u){return function (){return [u,n, ++m]}})( +new Date(),Math.random()*(1<<30)>>>0,unique()),genint=function (){var v=genval();
...
_ instanceof Array?(it=_[j],j<lj):(it=_, !j);
 ++j)this._append((itc=it.constructor)===String||itc===Number||itc===Boolean?new this.constructor(it):it)}},node_methods);
...
return head};
var compile=function (tree,environment){var vars=[],values=[],bindings=merge({},environment||{},tree.bindings()),s=gensym();
...
var macro_array_push=Array.prototype.push,macro_try_match=function (pattern,t){if (pattern.data==='_')return [t];
if (pattern.data!==t.data||pattern.length!==t.length)return null;
for (var i=0,l=pattern.length,wildcards=[],match=null;
i<l;
 ++i)if (match=macro_try_match(pattern[i],t[i]))macro_array_push.apply(wildcards,match);
else return null;
return wildcards},macro_expand_naive=function (t,macros,expanders,context){return t.rmap(function (n){for (var i=macros.length-1,macro,match,replacement;
i>=0&&(macro=macros[i]);
 --i)if ((match=macro_try_match(macro,n))&&(replacement=expanders[i].apply(context,match)))return replacement})};
var jit_macroexpander=(function (){var resolve_tree_path=function (tree,path){for (var i=0,l=path.length;
i<l;
...
return r},path_reference_array_template=parse('[_elements]'),generate_path_reference_array=function (variables,paths){for (var refs=[],i=0,l=paths.length;
i<l;
 ++i)refs.push(generate_path_reference(variables,paths[i]));
return path_reference_array_template.replace({_elements:refs.length>1?new syntax_node(',',refs):refs[0]})},macroexpander_invocation_template=parse('if (result = _expander.apply(this, _path_reference_array)) return result'),generate_macroexpander_invocation=function (pattern_data,pattern,variables){return macroexpander_invocation_template.replace({_expander:new ref(pattern_data[pattern.id()].expander),_path_reference_array:generate_path_reference_array(variables,pattern_data[pattern.id()].wildcard_paths)})},length_reference_template=parse('_value.length'),data_reference_template=parse('_value.data'),generate_partitioned_switch=function (trees,visited,variables,pattern_data){var path=next_path(visited,trees),partitions=visit_path(path,visited,trees),lengths={},length_pairs=[];
...
for (var length_cases=new syntax_node(';'),i=0,l=length_pairs.length,pair;
i<l;
 ++i){for (var data_cases=new syntax_node(';'),length=(pair=length_pairs[i])[0],values=pair[1],j=0,lj=values.length,p,v;
j<lj;
...
lj&&length_cases.push(partition_branch_template.replace({_value:''+length_pairs[i][0],_body:partition_template.replace({_value:data_reference,_cases:data_cases})}))}return single_macro_attempt_template.replace({_body:new syntax_node(';',path_reference_variable,length_cases.length?partition_template.replace({_value:length_reference,_cases:length_cases}):[])})},generate_unpartitioned_sequence=function (trees,variables,pattern_data){for (var r=new syntax_node(';'),i=0,l=trees.length;
i<l;
 ++i)r.push(generate_macroexpander_invocation(pattern_data,trees[i],variables));
return r},generate_decision_tree=function (trees,path,visited,variables,pattern_data){for (var r=new syntax_node(';'),sts=split_treeset_on_specification(trees,pattern_data,visited),i=0,l=sts.length;
i<l;
...
return compile(pattern_match_function_template.replace({_body:generate_decision_tree(rps,null,null,empty_variable_mapping_table(),pattern_data(rps,res))}))}})(),macro_expand_baked=function (t,f,context){return t.rmap(function (n){return f.call(context,n)})};
var precompile=(function (){var function_pattern=parse('function (_) {_}'),function_gensym_template=parse('function (_args, _gensym) {_body}'),mark_function_macro=function (references){return function (args,body){var s=gensym(),result=function_gensym_template.replace({_args:args,_gensym:s,_body:body});
return references[s]={tree:result},result}},annotate_functions_in=function (tree,references){return macro_expand_naive(tree,[function_pattern],[mark_function_macro(references)],null)},gensym_detection_pattern=parse('function (_, _) {_}'),wrapped_compile=function (original,references){return function (tree,environment){var matches=tree.match(gensym_detection_pattern),k=matches&&matches[1].data;
...
return original.call(this,tree,environment)}},already_compiled_template=parse('caterwaul.precompiled_internal(_x)'),signal_already_compiled=function (tree){return already_compiled_template.replace({_x:tree})},closure_template=parse('(function () {_vars; return (_value)})()'),closure_variable_template=parse('var _var = _value'),closure_null_template=parse('null'),syntax_ref_template=parse('caterwaul.parse(_string)'),caterwaul_ref_template=parse('caterwaul.clone(_string)'),syntax_ref_string=function (ref){return '\''+ref.serialize().replace(/\\/g,'\\\\').replace(/\n/g,'\\n').replace(/'/g,'\\\'')+'\''},caterwaul_ref_string=function (has){var ks=[];
for (var k in has)own.call(has,k)&&ks.push(k);
return '\''+ks.join(' ')+'\''},serialize_ref=function (ref){if (ref.value instanceof syntax_node)return syntax_ref_template.replace({_string:syntax_ref_string(ref)});
...
else throw new Error('syntax ref value is not serializable: '+ref.value)},variables_for=function (tree){var names=[],values=[];
tree.reach(function (n){if (n&&n.binds_a_value)names.push(n.data),values.push(n.value)});
for (var vars=[],i=0,l=names.length;
i<l;
 ++i)vars.push(closure_variable_template.replace({_var:names[i],_value:values[i]}));
return names.length?new syntax_node(';',vars):closure_null_template},precompiled_closure=function (tree){return closure_template.replace({_vars:variables_for(compiled),_value:compiled})},precompiled_function=function (tree){return signal_already_compiled(precompiled_closure)},substitute_precompiled=function (references){return function (args,gensym,body){return references[gensym.data]&&precompiled_function(references[gensym.data].compiled)}},perform_substitution=function (references,tree){return macro_expand_naive(tree,[gensym_detection_pattern],[substitute_precompiled(references)],null)},annotated_caterwaul=function (caterwaul,references){return caterwaul.clone().method('compile',wrapped_compile(caterwaul.compile,references))},trace_execution=function (caterwaul,f){var references={},annotated=references.annotated=annotate_functions_in(parse(f),references);
...
for (var k in f.attributes)has(f.attributes,k)&&g.associate(k,f.attributes[k],f[k])})},replica=se(function (){return copy_of({behaviors:{method:function (v){return bind(v,this)}}}).behavior('field').behavior('shallow',shallow_copy)},function (f){f.init=f}),configurable=function (f){return f.shallow('configurations',{}).shallow('has',{}).method('configuration',function (name,f){this.configurations[name]=f;
return this}).method('namespace',function (s){return this[s]||this.shallow(s,{})[s]}).method('clone',function (){return arguments.length?this.clone().configure.apply(null,arguments):copy_of(this)}).method('configure',function (){for (var i=0,l=arguments.length,_;
_=arguments[i],i<l;
...
return this})};
var macroexpansion=function (f){return f.shallow('macro_patterns',[]).method('macro',function (pattern,expander){if ( !expander.apply)throw new Error('macro: Cannot define macro with non-function expander');
else return this.macro_patterns.push(pattern),this.macro_expanders.push(expander),this}).shallow('macro_expanders',[]).method('macroexpand',function (t){return this.baked_macroexpander?macro_expand_baked(t,this.baked_macroexpander,this):macro_expand_naive(t,this.macro_patterns,this.macro_expanders,this)}).method('bake_macroexpander',function (){return this.method('macro',function (){throw new Error('Cannot define new macros after baking the macroexpander')}).field('baked_macroexpander',jit_macroexpander(this.macro_patterns,this.macro_expanders))}).method('rmacro',function (pattern,expander){if ( !expander.apply)throw new Error('rmacro: Cannot define macro with non-function expander');
else return this.macro(pattern,function (){var t=expander.apply(this,arguments);
return t&&this.macroexpand(t)})})},composition=function (f){return f.shallow('after_functions',[]).method('after',function (){if (arguments.length){for (var i=0,l=arguments.length;
i<l;
 ++i)this.after_functions.push(arguments[i]);
return this}else return this.after_functions})},caterwaul_core=function (f){return configurable(f).configure(macroexpansion,composition).method('tconfiguration',function (configs,name,f,bindings){this.configurations[name]=this.clone(configs)(f,bindings);
return this}).field('syntax',syntax_node).field('ref',ref).field('parse',parse).field('compile',compile).field('gensym',gensym).field('map',map).field('self',self).field('unique',unique).field('macroexpansion',macroexpansion).field('replica',replica).field('configurable',configurable).field('caterwaul',caterwaul_core).field('decompile',parse).field('composition',composition).field('global',function (){return caterwaul_global}).field('precompiled_internal_table',{}).method('precompile',precompile).method('precompiled_internal',function (f){var k=gensym();
this.precompiled_internal_table[k]=f;
return k}).method('init',function (f,environment){if (f.constructor===String&&this.precompiled_internal_table[f])return this.precompiled_internal_table[f];
...
caterwaul.tconfiguration('std macro','opt.unroll',function (){this.rmacro(qs[opt.unroll[_,_][_]],fn[variable,iterations,body][with_gensyms[l,rs,es,j][qg[function (l){for (var rs=l>=0&&l>>3,es=l>=0&&l&7,_i_=0;
_i_<es;
 ++_i_)_body_;
for (var j=0;
j<rs;
